...
const SellerBids = () => {
  ...
  const [selectedProductId, setSelectedProductId] = useState(null);
  const [loading, setLoading] = useState(true);
  const [isSessionValid, setIsSessionValid] = useState(false); // Added session state
  ...
  useEffect(() => {
    const validateSession = () => {
      ...
      console.log('[Auth Check] Validating session for SellerBids');
  ...
  useEffect(() => {
    if (!isSessionValid) return; // Don't fetch if session invalid
  ...
  const handleFetchBids = async (productId) => {
    if (!validateSession()) return; // Session check
  ...
  // Don't render content if session is invalid
  if (!isSessionValid) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#e6f2f5] via-[#f0f8fa] to-[#faf6e9]">
        <div className="text-center p-8 bg-white/90 rounded-xl shadow-xl border border-[#016A6D]/20">
          <div className="animate-spin rounded-full h-16 w-16 border-t-4 border-[#016A6D] mx-auto mb-4"></div>
          <p className="text-[#043E52] text-lg">Verifying your session...</p>
        </div>
      </div>
    );
  }
  ...
  const renderProductCard = (product) => (
    ...
    >
      ...
      <div className="md:w-2/3 p-4 flex flex-col justify-between">
        <div>
          <h2 className="text-xl font-bold mb-1 text-[#043E52]">{product.name}</h2>
          <p className="text-gray-600 text-sm mb-0.5">
            Status: <span className={`capitalize font-semibold ${product.status === 'draft' ? 'text-[#FFAA5D]' : 'text-[#016A6D]'}`}>{product.status}</span>
          </p>
          <p className="text-gray-600 text-sm mb-0.5">
          ...
          <p className="text-[#043E52] text-base mt-2 font-semibold">Starting Price: PKR {product.startingPrice.toLocaleString()}</p>
        </div>
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <div className="ml-auto flex space-x-2">
            <button
              onClick={() => handleViewProductDetails(product._id)}
              className="inline-flex items-center px-3 py-1.5 bg-[#016A6D] text-white rounded-md text-xs font-medium hover:bg-[#043E52] transition-colors"
            >
              View Details
            </button>
            <button
              ...
              className="inline-flex items-center px-3 py-1.5 bg-[#E16A3D] text-white rounded-md text-xs font-medium hover:bg-[#FFAA5D] transition-colors"
            >
              <ListCollapse size={16} className="mr-1" /> See Bids
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        {displayBids && (
          <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-2xl p-6 w-full max-w-xl relative shadow-2xl max-h-[90vh] overflow-hidden flex flex-col font-sans">
              <button
                onClick={() => {
                  setDisplayBids(false);
                  setBids(null);
              ...
              <h2 className="text-xl font-bold mb-4 text-center text-[#043E52]">Live Bids for Product {selectedProductId ? selectedProductId.substring(0, 8) + '...' : ''}</h2>
                      ...
                      >
                        ...
                        <div className="flex gap-2 mt-2 sm:mt-0">
                          <button
                            ...
                            className="bg-green-500 hover:bg-green-600 text-white p-2 rounded-lg flex items-center justify-center transition-colors shadow-md text-sm"
                            title="Accept Bid"
                          >
                            <Check size={18} />
                          </button>
                          <button
                            onClick={() => handleReject(bid._id)}
                            className="bg-red-500 hover:bg-red-600 text-white p-2 rounded-lg flex items-center justify-center transition-colors shadow-md text-sm"
                            title="Reject Bid"
                          >
                            <X size={18} />
                          </button>
        ...
        <motion.h1 variants={itemVariants} className="text-3xl font-extrabold text-[#043E52] mb-8 text-center leading-tight">
          Manage Your Auctions & Bids
        </motion.h1>
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center py-16 bg-white rounded-xl shadow-md">
            <Loader2 className="w-12 h-12 text-[#016A6D] animate-spin" />
            <p className="ml-4 text-base text-[#043E52] mt-3">Loading your products...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {activeProducts.length > 0 ? (
              ...
              <motion.div variants={itemVariants} className="md:col-span-3 text-center py-8 bg-white rounded-xl shadow-md text-[#043E52] text-base border border-[#016A6D]/10">
                No active products found. Start a new listing!
              </motion.div>
            )}
          </div>
