...
const AllProducts = () => {
  ...
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [sortOption, setSortOption] = useState('');
  const [priceRange, setPriceRange] = useState(500000); // default to max so filtering works
  const [selectedCategories, setSelectedCategories] = useState([]);
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  ...
  // Ensure this matches the category names you send from homepage or check normalization below
  const categories = [
    { id: 1, name: 'Mobile Phones' },
    { id: 2, name: 'Laptops' },
    { id: 3, name: 'Camera Equipment' },
  ...
  // Apply filters whenever products load or filters change, but only after loading is false
  useEffect(() => {
    if (!loading) {
      applyFilters();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [products, selectedCategories, priceRange, sortOption, loading]);
  ...
  // MODIFY handleSort: only set sortOption
  const handleSort = (option) => {
    setSortOption(option);
  };
  ...
  const applyFilters = () => {
    ...
    if (selectedCategories.length > 0) {
      // Normalize comparison if needed: here using exact match.
      filtered = filtered.filter((product) =>
        selectedCategories.includes(product.category)
        // If your backend returns different casing/spaces, use:
  ...
  // OPTIONAL: Debug logging to console (remove in production)
  // useEffect(() => {
